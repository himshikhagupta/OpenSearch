...
pub extern "C" fn native_heap_count() -> i32 {
    all_plugin_stats().len() as i32
}

/// Writes the plugin name at `index` into `buf` (up to `buf_len` bytes).
/// Returns the actual byte length, or -1 if index is out of range.
#[no_mangle]
pub unsafe extern "C" fn native_heap_name(index: i32, buf: *mut u8, buf_len: i32) -> i32 {
    let stats = all_plugin_stats();
    let i = index as usize;
    if i >= stats.len() || buf.is_null() {
        return -1;
    }
    let name = stats[i].name.as_bytes();
    let n = name.len().min(buf_len as usize);
    std::ptr::copy_nonoverlapping(name.as_ptr(), buf, n);
    n as i32
}

/// Returns `used` bytes for the plugin heap at `index`, or -1 if out of range.
#[no_mangle]
