...
impl MemoryPool for QueryMemoryPool {
    ...
    fn try_grow(&self, reservation: &MemoryReservation, additional: usize) -> Result<(), DataFusionError> {
        match self.inner.try_grow(reservation, additional) {
            ...
            // No fallback — if the inner pool rejects, respect that decision.
            // The pool limit should be configured large enough for the workload.
            Err(e) => Err(e),
        }
    }
...
// ---------------------------------------------------------------------------
// Component-level memory budget for the DataFusion plugin
// ---------------------------------------------------------------------------
//
// The DF MemoryPool tracks cooperative allocations (batch buffers, hash tables,
// sort state). Some memory escapes this tracking: struct overhead, metadata,
// non-cooperative allocations. We estimate total plugin memory as:
//
//   estimated = pool.reserved() * (1 + overhead_buffer_percent / 100)
//
// If this estimate exceeds the component limit, new queries are rejected.
// Both the limit and the buffer percentage are configurable cluster settings.

static COMPONENT_LIMIT: OnceLock<usize> = OnceLock::new();
static OVERHEAD_BUFFER_PERCENT: OnceLock<usize> = OnceLock::new();

/// Set the component-level memory limit for the DataFusion plugin.
/// Called from Java at startup or on settings update.
pub fn set_component_limit(limit: usize) {
    COMPONENT_LIMIT.get_or_init(|| limit);
}

/// Set the overhead buffer percentage (e.g., 20 means 20% buffer).
/// Called from Java at startup or on settings update.
pub fn set_overhead_buffer_percent(percent: usize) {
    OVERHEAD_BUFFER_PERCENT.get_or_init(|| percent);
}

fn component_limit() -> usize {
    *COMPONENT_LIMIT.get().unwrap_or(&usize::MAX)
}

fn overhead_buffer_percent() -> usize {
    *OVERHEAD_BUFFER_PERCENT.get().unwrap_or(&20)
}

/// Estimate total plugin memory usage from the cooperative pool's reserved bytes
/// plus a configurable overhead buffer.
pub fn estimated_usage(pool: &dyn MemoryPool) -> usize {
    let reserved = pool.reserved();
    let buffer = overhead_buffer_percent();
    reserved + (reserved * buffer / 100)
}

/// Check whether the plugin has capacity to accept a new query.
/// Returns `Ok(())` if estimated usage is below the component limit,
/// or `Err(DataFusionError)` if the plugin should reject the request.
pub fn check_component_capacity(pool: &dyn MemoryPool) -> Result<(), DataFusionError> {
    let estimated = estimated_usage(pool);
    let limit = component_limit();
    if estimated >= limit {
        Err(DataFusionError::ResourcesExhausted(format!(
            "DataFusion plugin memory limit reached: estimated={}MB (reserved={}MB + {}% buffer), limit={}MB",
            estimated / (1024 * 1024),
            pool.reserved() / (1024 * 1024),
            overhead_buffer_percent(),
            limit / (1024 * 1024),
        )))
    } else {
        Ok(())
